...
<script setup>
  ...
  watch(
    ...
    async (newFullPath, oldFullPath) => {
      console.log("IN Issues.watch.refresh newFullPath=" + newFullPath + "; oldFullPath=" + oldFullPath);
      if((oldFullPath.includes("/prototype") || oldFullPath.includes("/base-unit")) && newFullPath.includes("/view-issues")){
        itemRef.value = props.item_ref;
        itemType.value = props.item_type;
        itemName.value = props.item_name;
        fetchIssues();
        issuesKey.value += 1;
      } else if(oldFullPath.includes("/add-issue") && newFullPath.includes("/view-issues")){ 
        fetchIssues();
        issuesKey.value += 1;
      }
      console.log("OUT Issues.watch.refresh");
    }
  );

  // Initialize data on mount of component
  onMounted(async () => {
    console.log('IN Issues.onMounted');
    itemRef.value = props.item_ref;
    itemType.value = props.item_type;
    itemName.value = props.item_name;
    fetchIssues();
    issuesKey.value += 1;
    console.log('OUT Issues.onMounted itemRef=' + itemRef.value + '; itemType=' + itemType.value);
  });

  // Go back to previous page
  const goBack = () => {
    console.log("IN Issues.goBack");
    router.back();
    console.log("OUT Issues.goBack");
  };

  // Add a note to the database
  const addIssue = () => {
    console.log("IN Issues.addIssue");
    router.push({name: 'add-issue', params: {item_type: itemType.value, item_ref: itemRef.value, item_name: itemName.value}}).catch(failure => {
      console.log('An unexpected navigation failure occurred:', failure);
    });
    console.log("OUT Issues.addIssue");
  }

  // Delete a issue from the database
  const deleteIssue = async (item) => {
    console.log("IN Issues.deleteIssue item=" + JSON.stringify(item));
    ...
    console.log("OUT Issuess.deleteIssue");
  };

  // Retrieve issuess from the database through a REST call.
